...
interface RegistrationRequestFilters {
  selectedStatus: string;
  selectedArea: string;
  searchTerm: string;
}

interface RegistrationRequestFilterProps {
  filters: RegistrationRequestFilters;
  areas: Array<{ id: number; name: string }>;
  onFiltersChange: (filters: RegistrationRequestFilters) => void;
  onSearch: () => void;
  onReset: () => void;
  resultsCount: number;
  isSearching?: boolean;
...
export const RegistrationRequestFilter: React.FC<RegistrationRequestFilterProps> = ({
  filters,
  areas,
  onFiltersChange,
  onSearch,
  ...
  const handleFilterChange = (key: keyof RegistrationRequestFilters, value: any) => {
    const newFilters = {
      ...filters,
      [key]: value
    };
      ...
      <div className="p-6 bg-gradient-to-r from-teal-50 to-cyan-50 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div className="text-lg font-bold text-gray-900">
            الفلاتر والتصفية المتقدمة لطلبات التسجيل
          </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="text-sm text-gray-600">
            <span className="font-medium text-teal-600">{resultsCount}</span> طلب
            {isSearching && (
              <span className="mr-2 rtl:ml-2 rtl:mr-0 text-teal-600">
                (جاري التحديث...)
              </span>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              <h4 className="text-sm font-semibold text-gray-900 mb-4 flex items-center">
                <div className="w-2 h-2 bg-teal-500 rounded-full ml-2 rtl:mr-2 rtl:ml-0"></div>
                البحث العام
              </h4>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-2">حالة الطلب</label>
                  <select
                ...
                <div className="flex flex-wrap gap-2">
                  {filters.searchTerm && (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-teal-100 text-teal-800">
                      ...
                      <button
                        ...
                        className="mr-1 rtl:ml-1 rtl:mr-0 hover:bg-teal-200 rounded-full p-0.5"
                      >
                        <X className="h-3 w-3" />
                      </button>
                    </span>
                  ...
                  {filters.selectedStatus && (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      الحالة: {
                        filters.selectedStatus === 'pending' ? 'قيد المراجعة' :
                        filters.selectedStatus === 'approved' ? 'تمت الموافقة' :
                        filters.selectedStatus === 'rejected' ? 'مرفوض' : filters.selectedStatus
                      }
                      <button
                        type="button"
            ...
            <div className="bg-teal-50 p-4 rounded-lg border border-teal-200">
              <div className="text-sm text-teal-800">
                ✅ <strong>الفلاتر الذكية لطلبات التسجيل:</strong> جميع الفلاتر تعمل فوراً عند التغيير مع إمكانية البحث بالحالة والمنطقة!
              </div>
            </div>
          </div>
        </div>
